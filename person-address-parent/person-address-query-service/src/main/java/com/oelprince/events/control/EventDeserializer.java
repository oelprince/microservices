...
public class EventDeserializer implements Deserializer<PersonAddressEvent> {
    ...
    public PersonAddressEvent deserialize(final String topic, final byte[] data) {
        try (ByteArrayInputStream input = new ByteArrayInputStream(data)) {
            ...
            logger.severe("Person-Address query service - Could not deserialize event: " + e.getMessage());
            throw new SerializationException("Person-Address query Service - Could not deserialize event", e);
        }
    }
